...
import { fetchDashboardStats, getStoredUser } from "../services/api";
import { EmptyState, ErrorState, formatDelta, formatRelativeFr } from "../components/ApiState";
    ...
    <div className="space-y-8 animate-pulse">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="luxury-card p-5">
            <div className="h-3 w-28 bg-[var(--bg-app)] rounded mb-4" />
            <div className="h-8 w-16 bg-[var(--bg-app)] rounded" />
            <div className="h-4 w-20 bg-[var(--bg-app)] rounded mt-4" />
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="luxury-card p-4 h-20 bg-[var(--bg-app)]/40" />
          ))}
        </div>
        <div className="lg:col-span-2 luxury-card p-6 h-72">
          <div className="h-4 w-40 bg-[var(--bg-app)] rounded mb-6" />
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-12 bg-[var(--bg-app)] rounded-xl" />
            ))}
          </div>
        </div>
      </div>
...
export default function Screen2Dashboard({ onStartDictation, onNavigate, isOffline, user: userProp }) {
  ...
  const load = useCallback(async ({ soft = false } = {}) => {
    if (isOffline) {
      setError("Serveur inaccessible. Impossible de charger le tableau de bord.");
      setLoading(false);
      setRefreshing(false);
      return;
    }
  ...
  const practitioner = stats?.practitioner || userProp || storedUser;
  const cards = [
    {
      label: "Consultations du jour",
      value: String(stats?.consultations_today ?? 0),
      delta: formatDelta(stats?.consultations_delta ?? 0),
      icon: "consult",
      color: "navy",
    },
    {
      label: "Patients suivis",
      value: String(stats?.patients_followed ?? 0),
      delta: formatDelta(stats?.patients_delta ?? 0),
      icon: "patients",
      color: "gold",
    },
    {
      label: "Transmissions SIH",
      value: `${stats?.transmission_rate ?? 0}%`,
      delta: formatDelta(stats?.transmission_delta ?? 0),
      icon: "transmit",
      color: "green",
    },
    {
      label: "Dictées en attente",
      value: String(stats?.pending_dictations ?? 0),
      delta: formatDelta(stats?.pending_dictations_delta ?? 0),
      icon: "dictation",
      color: "blue",
    },
  ];
  ...
  const isEmptyDashboard =
    ...
    !error &&
    stats &&
    (stats.consultations_today ?? 0) === 0 &&
    (stats.patients_followed ?? 0) === 0 &&
    (stats.pending_dictations ?? 0) === 0 &&
      ...
      <div className="relative luxury-card p-6 lg:p-8 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 relative">
          <div>
            ...
            <h1 className="text-3xl lg:text-4xl font-bold">Bonjour, {doctorDisplayName(practitioner)}</h1>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() => go("screen3")}
              disabled={isOffline}
              className="btn-gold text-base px-6 py-3.5 disabled:opacity-50"
            >
        ...
        <>
          {isEmptyDashboard ? (
            <EmptyState
              ...
              message="Créez un patient et lancez une dictée pour alimenter votre tableau de bord."
            />
          ) : null}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="luxury-card p-6">
                ...
                <div className="space-y-3">
                  {recentActivity.length === 0 ? (
                    <EmptyState title="Aucune activité récente" message="Les consultations apparaîtront ici dès qu'elles seront créées." />
                  ) : (
                    recentActivity.map((a, i) => (
                      <div key={a.consultation_id || i} className="flex items-center gap-4 p-3 rounded-xl hover:bg-[var(--bg-app)] transition">
                        ...
                        {a.code ? <span className="lux-badge badge-blue hidden sm:inline-flex">{a.code}</span> : null}
                        <span className="text-xs text-[var(--text-muted)] whitespace-nowrap">{formatRelativeFr(a.time)}</span>
                      </div>
                    ))
                  )}
                </div>
